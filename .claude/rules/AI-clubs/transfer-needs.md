...
candidate sellers and never buy from the human sell list. General-purpose, per-club freeze —
nothing in the live pipeline sets it today.
...
unchanged — every club sits out, not just a listed set. Only the start-kit pre-simulation
(`presimulatePreStart` → `advanceOneDay(..., { marketFrozen: true })`) uses it. Every club in
...
already sold before anyone picked the club. The human club is still excluded separately
