 ...
 *
 * Run:  bun scripts/bench-advance-day.ts [--days 14]
 *
...
type IoCat = "squadRead" | "squadWrite" | "squadList" | "other";
const io = {
  ...
  squadWrites: 0,
  listAllSquads: 0,
  busyMs: { squadRead: 0, squadWrite: 0, squadList: 0, other: 0 } as Record<IoCat, number>,
};
const inflight: Record<IoCat, { n: number; since: number }> = {
  squadRead: { n: 0, since: 0 },
  squadWrite: { n: 0, since: 0 },
  squadList: { n: 0, since: 0 },
  other: { n: 0, since: 0 },
};
...
async function runDays(saveId: string): Promise<DayRow[]> {
  const service = new SaveService(countingDAL(new FileSystemDAL()));
  const rows: DayRow[] = [];
...
function printTable(title: string, rows: DayRow[]): void {
  ...
  for (const [label, rs] of [["user-league matchdays", userDays], ["other days", otherDays]] as const) {
    ...
    const accounted = avg(rs.map((r) => keys.reduce((s, k) => s + r.profile[k]!, 0)));
...
try {
  ...
  const rows = await runDays(saveId);
  printTable(`unbuffered (FileSystemDAL), ${DAYS} days`, rows);
} finally {
  if (saveId) {
